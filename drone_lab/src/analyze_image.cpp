...
// might be used for displaying state of the drone
...
void control(const ardrone_autonomy::Navdata& msg_in)
{
  ...
  z = msg_in.rotZ;
  h = msg_in.altd;
  // uncomment to get all the data printed
...
void find_image_center(cv::Mat* img, int hue, int saturation, int value) {
  int width = img->cols;
  int height = img->rows;
...
void imageCallbackBottom(const sensor_msgs::ImageConstPtr& msg)
{
  try
  {
    ...
    find_image_center(&frame_copy, 56, 149, 203);
  }
  catch (cv_bridge::Exception& e)
  {
    ROS_ERROR("Could not convert from '%s' to 'bgr8'.", msg->encoding.c_str());
